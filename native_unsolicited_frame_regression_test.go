...
func TestNativeReaderRejectsUnknownDataRequestID(t *testing.T) {
	client, server := net.Pipe()
	exec := NewNativeExecutor("unused")
	exec.mu.Lock()
	exec.conn = client
	...
	writer := bufio.NewWriter(server)
	if err := writeNativeTestResponse(writer, nativeFrame{
		laneID: 1, opcode: nativeOpGet, requestID: 99,
	}, nativeStatusOK, []byte("orphan")); err != nil {
		t.Fatal(err)
	}
	select {
	case <-done:
	case <-time.After(100 * time.Millisecond):
		t.Fatal("native reader accepted an unknown data request_id")
	}
}
