...
func TestScheduleFireDueCannotExceedRequestedLimit(t *testing.T) {
	client := NewClientWithExecutor(&fakeExecutor{value: map[string]any{
		"claimed": int64(2), "fired": int64(2), "skipped": int64(0), "coalesced": int64(0), "errors": []any{}, "last_target_id": "target-2",
	}})
	if _, err := client.ScheduleFireDue(context.Background(), ScheduleFireDueOptions{
		Limit: Int(1),
...
func TestScheduleListCannotExceedRequestedCount(t *testing.T) {
	client := NewClientWithExecutor(&fakeExecutor{value: []any{
		map[string]any{"id": "one", "kind": "interval", "state": "active", "catchup_policy": "fire_once"},
		map[string]any{"id": "two", "kind": "interval", "state": "active", "catchup_policy": "fire_once"},
	}})
...
func TestScheduleResponsesCannotExceedV080DefaultLimit(t *testing.T) {
	client := NewClientWithExecutor(&fakeExecutor{value: map[string]any{
		"claimed": int64(101), "fired": int64(101), "skipped": int64(0), "coalesced": int64(0), "errors": []any{}, "last_target_id": "target-101",
	}})
...
func TestScheduleFireDueRejectsTooManyErrorsBeforeItemDecoding(t *testing.T) {
	client := NewClientWithExecutor(&fakeExecutor{value: map[string]any{
		"claimed": int64(1), "fired": int64(0), "skipped": int64(0), "coalesced": int64(0),
		"errors": []any{"malformed", "malformed"},
	}})
	_, err := client.ScheduleFireDue(context.Background(), ScheduleFireDueOptions{Limit: Int(1)})
